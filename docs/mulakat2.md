...
- **Kayıt/giriş UX + Google ile giriş + misafir denemesi + tanıtım sayfası (yapıldı, 2026-09-24):** Kullanıcı adı alanı kaldırıldı, kayıt/giriş yalnızca e-posta+parola (`username=email` içeride tutulur, eski kullanıcılar kendi kullanıcı adlarıyla da girebilir — `accounts/auth_backends.py::EmailAuthBackend`). Django'nun varsayılan uzun parola kuralları listesi kaldırıldı, kısa ipucu + yalnızca hatada görünen mesajlar kondu. Google Identity Services ile "Google ile giriş" eklendi (yeni paket yok, mevcut `google-auth` kullanıldı; sunucu tarafı sır gerekmez, yalnızca `GOOGLE_CLIENT_ID`); Google e-postayı doğruladığı için ayrıca e-posta doğrulaması istenmez. **Misafir denemesi** (`/deneme/`): hesap açmadan, IP başına günde 3 kez, 2 soruluk kısa bir mülakat (oturumda tutulur, DB'ye yazılmaz), sonunda puan + kayıt CTA'sı. Ana sayfaya "Nasıl çalışır", pozisyon listesi, özellikler, SSS (native `<details>`) ve site geneli footer (iletişim/GitHub/gizlilik/KVKK) eklendi; her sayfaya `meta description`, ana sayfaya Open Graph/Twitter Card + `static/img/og-image.png` (Pillow ile tek seferlik üretildi, projeye bağımlılık eklemez). `/gizlilik/` ve `/kvkk/` taslak yasal sayfalar + kayıtta zorunlu "Aydınlatma Metni'ni okudum" onay kutusu. Yeni env: `GOOGLE_CLIENT_ID` (opsiyonel). Migration yok (mevcut `User.is_active`/`username` alanları kullanıldı).
...
- **Süreler:** Bir model denemesi en fazla 30 sn sürer (`_CALL_TIMEOUT_SECONDS`; sağlıklı modeller 5 soruyu 5–20 sn'de üretir). **SDK'nın kendi yeniden denemesi kapalıdır (`attempts=0`):** bu SDK zaman aşımını da "bağlantı hatası" sayıp yeniden deniyor (`timeout=20` ile tek çağrı 61 sn sürdü), süre 3 katına çıkıp Vercel sınırını aşabiliyordu. Yeniden deneme işini uygulama yapar: yavaş ya da hata veren model yerine yedek zincirdeki sıradaki model denenir (aşağıya bak). Son cevapta iki çağrı (değerlendirme + özet) arka arkaya çalıştığı için `vercel.json`'da `maxDuration` 120 sn'dir (Fluid compute açıkken Hobby planında varsayılan ve en fazla 300 sn). Her çağrının süresi `logger.info` ile kaydedilir; canlıda gerçek gecikmelere göre ayar yapılır.
- **Yavaş ya da hata veren model → sıradaki model:** Bir model 30 sn içinde yanıt vermezse (zaman aşımı; Google tarafında geçici yavaşlık, ücretsiz katmanda görüldü) ya da 5xx/408 dönerse yedek zincirdeki bir sonraki model denenir; tıpkı 429 (kota) gibi. Yavaş çıkan model sonraki 5 dakika boyunca listenin sonuna alınır (atlanmaz, yavaşlık geçicidir), böylece her istek önce onu beklemez. Bir çağrının yedekler dahil toplam süresi bütçeyle sınırlıdır (soru üretme 90 sn = 3 deneme, cevap değerlendirme 60 sn = 2 deneme, genel özet 40 sn = 1 deneme; kalan süre 12 sn'nin altındaysa yeni model denenmez; son cevapta değerlendirme + özet toplamı Vercel'in 120 sn sınırının altında). Hiçbir model yanıt vermezse kullanıcıya "Yapay zeka servisi şu an yavaş ya da yoğun. Birkaç dakika sonra tekrar dene." gösterilir (soru üretmede form, cevap gönderimde 503 + "yazdığın cevap korundu"). 429, zaman aşımı ve 5xx dışındaki hatalar zinciri durdurur. Gerçek testte (2026-09-21, Google yavaşken) 3.5-flash 31 sn'de zaman aşımına uğrayınca 3.1-flash-lite 5.4 sn'de 5 soruyu üretti (toplam 38 sn).
...
| `/deneme/baslat/`, `/deneme/`, `/deneme/cevap/`, `/deneme/sonuc/` | Misafir denemesi (hesapsız, oturum bazlı, IP başına 3/gün) |
| `/gizlilik/`, `/kvkk/` | Gizlilik Politikası, KVKK Aydınlatma Metni (taslak) |
